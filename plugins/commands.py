...
# Ask Doubt on telegram @KingVJ01
    ...
    if not files_:
        ...
        try:
            ...
            title = '@Chatkanhabot  ' + ' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), file.file_name.split()))
            ...
            k = await msg.reply(f"<b><u>❗️❗️❗️IMPORTANT❗️️❗️❗️</u></b>\n\nThis Movie File/Video will be deleted in <b><u>{AUTO_DELETE} mins</u> 🫥 <i></b>(Due to Copyright Issues)</i>.\n\n<b><i>Please forward this File/Video to your Saved Messages and Start Download there. THIS BOT FAST ⏩ DOWNLOAD AND WATCH ONLINE 👀 NOT WORKING SORRY 😐 NO PROBLEM MY NEW BOT USE - @FILETOSTREAMKANHABOT</b>",quote=True)
            await asyncio.sleep(AUTO_DELETE_TIME)
            await msg.delete()
            await g.delete()
